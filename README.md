...
graph TD;
	...
	filter_by_type_reverse(filter_by_type_reverse)
	...
	check_code_match -.-> generate_reverse_rationale;
	check_code_match -.-> infer_secondary_evidence;
	check_code_match -.-> infer_secondary_evidence_reverse;
	...
	discover_as_is_candidates -.-> filter_by_type;
	discover_as_is_candidates -.-> handle_exception;
	discover_to_be_candidates -.-> filter_by_type_reverse;
	discover_to_be_candidates -.-> handle_exception;
	...
	filter_by_type_reverse -.-> check_code_match;
	filter_by_type_reverse -.-> handle_exception;
	...
	infer_secondary_evidence_reverse --> judge_and_rank;
	judge_and_rank -.-> generate_rationale;
	judge_and_rank -.-> generate_reverse_rationale;
	judge_and_rank -.-> handle_exception;
	judge_and_rank -.-> request_clarification;
	lookup_mapping_candidates -.-> discover_as_is_candidates;
	lookup_mapping_candidates -.-> filter_by_type;
	lookup_mapping_candidates -.-> handle_exception;
	lookup_reverse_mapping -.-> discover_to_be_candidates;
...
**후보 자동 탐색(2026-09-09 추가)**: `lookup_mapping_candidates`/`lookup_reverse_mapping`이 매핑정의서에 등록이 없어서 `no_match`가 나올 상황(컬럼 자체는 실존)이면, 곧바로 실패시키지 않고 `discover_as_is_candidates`/`discover_to_be_candidates`가 반대편 스키마 **전체**를 후보 풀로 삼아 기존 판정 파이프라인(`filter_by_type`→`check_code_match`→`infer_secondary_evidence`→`judge_and_rank`)에 그대로 태웁니다. 다만 사람이 사전에 검증해 둔 등록 매핑이 아니라서 **더 엄격한 임계값**(`DISCOVERY_LOW_CONFIDENCE_THRESHOLD=0.75`, `DISCOVERY_CONFIDENCE_GAP_THRESHOLD=0.20` — 등록 매핑 기준 0.5/0.10보다 높음)을 적용하고, `request_clarification` 되묻기 루프는 타지 않습니다(등록 안 된 걸 스스로 찾는 것이므로 한 번에 확신이 안 서면 사람에게 되묻지 않고 정직하게 `discovery_inconclusive`로 종료). confirmed로 나오면 근거 문장에 "매핑정의서에 등록된 게 아니라 자동으로 탐색해 찾은 결과"임을 명시합니다. 정방향(`discover_as_is_candidates`→`filter_by_type`)과 역방향(`discover_to_be_candidates`→`filter_by_type_reverse`→`infer_secondary_evidence_reverse`) 양쪽 다 지원합니다.
...
- [x] 후보 자동 탐색(`src/discovery.py`, 2026-09-09 추가) — 매핑정의서에 등록이 없는 컬럼(정방향/역방향 모두)을 `no_match`로 바로 끝내지 않고, 반대편 스키마 전체를 후보로 삼아 기존 판정 파이프라인을 더 엄격한 임계값으로 재사용. 되묻기 루프 없이 confirmed 또는 정직한 `discovery_inconclusive`로 단판 종료
...
.venv/bin/python -m src.graph ACC_WHT_AGG.lbr_pay_amt   # SC-001: 후보 자동 탐색(정방향, 등록 안 된 컬럼) — 탐색 성공
.venv/bin/python -m src.graph ACC_WHT_AGG.data_quality_flag_cd   # SC-001: 후보 자동 탐색 — 정직한 탐색 실패
.venv/bin/python -m src.graph "INT_WHT.rate_cd가 TO-BE 어디로 매핑돼?"   # SC-001: 후보 자동 탐색(역방향) — 탐색 성공
...
│   ├── evidence.py          Node: infer_secondary_evidence / infer_secondary_evidence_reverse
│   ├── judge.py             Node: judge_and_rank
│   ├── discovery.py         Node: discover_as_is_candidates / discover_to_be_candidates (후보 자동 탐색)
