...
| `confirmed` | `wht_tax_amt` | 후보 둘 다 AS-IS 설명 있음 → 임베딩 유사도(`description_similarity`)로 점수화 → `judge_and_rank`가 점수차(confidence_gap)로 확정 |
| `confirmed` | `tax_rate` / `payee_biz_no` / `div_payee_nm` | 후보에 AS-IS 설명 없음 → LLM 자체추론(`self_inference`, 컬럼명·타입·샘플값 기반)으로 점수화 → `judge_and_rank`가 점수차로 확정 |
